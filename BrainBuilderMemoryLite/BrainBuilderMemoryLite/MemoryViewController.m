...
-(void) AlertView3
{
    ...
    alertDialog = [[UIAlertView alloc]
                   ...
                   message:@"You have completed the free version would you like to upgrade?"
                   delegate:self
                   cancelButtonTitle:@"Go Home"
                   otherButtonTitles:@"Upgrade", @"Play Again", nil];
    [alertDialog show];
}
...
clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    else if ([buttonTitle isEqualToString:@"Go Home"])
    {
        [self.navigationController popToRootViewControllerAnimated:YES];
    }
    else if ([buttonTitle isEqualToString:@"Play Again"])
    {
        round = 1;
        level = 3;
        assist = 0;
...
- (void)viewDidLoad
{
    ...
    self.LivesLabel.text = [[NSString alloc] initWithFormat:@"%i", (4-numbWrong)];
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    int savedRound = [userDefaults integerForKey:ROUND];
    if (savedRound < 1)
    {
        round = 1;
    }
