...
@implementation TrigCustomSetUp
...
@synthesize quad3;
@synthesize quad4;
@synthesize funcDetails;
@synthesize logSettings;
@synthesize choice;

-(IBAction) loadCustomGamePage:(id)sender
{
    if ((cos.on || sin.on || tan.on || csc.on || sec.on || cot.on) && (quad1.on || quad2.on || quad4.on || quad4.on) && (choice.selectedSegmentIndex == 0))
    {
        [self helper];
        TrigCustomGame *customGame = [[TrigCustomGame alloc] initWithNibName:@"TrigCustomGame" bundle:nil];
        self.navigationItem.title = @"Back";
        [self.navigationController pushViewController:customGame animated:YES];
    }
    else if (choice.selectedSegmentIndex == 1)
    {
        TrigCustomLog *customGame3 = [[TrigCustomLog alloc] initWithNibName:@"TrigCustomLog" bundle:nil];
        self.navigationItem.title = @"Back";
        [self.navigationController pushViewController:customGame3 animated:YES];
...
-(IBAction) exchangeDetailView:(id)sender
{
    ...
    {
        [funcDetails insertSubview:logSettings.view atIndex:76];
        
    }
    else if (choice.selectedSegmentIndex == 0)
    {
        [logSettings.view removeFromSuperview];
    }
}
...
-(void) helper
{
    ...
    [customGame2 learnSettings:sin2: cos2: tan2: csc2: sec2: cot2: quad12: quad22: quad32: quad42];
}
